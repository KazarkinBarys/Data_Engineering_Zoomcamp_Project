...
WITH  
temp_table AS (SELECT * FROM {{ source('MVC_C','MVC_C_2021') }}),
temp_table2 AS (SELECT * FROM {{ source('MVC_V','MVC_V_2021') }}),
temp_table3 AS (SELECT * FROM {{ source('MVC_P','MVC_P_2021') }}),
...
vhc_year AS (SELECT vhc_year, vhc_year_am, ROW_NUMBER() OVER() AS ind FROM(SELECT vhc_year, count(*) AS vhc_year_am FROM temp_table2 WHERE vhc_year IS NOT NULL AND vhc_year BETWEEN 1900 AND 2021 GROUP BY vhc_year ORDER BY 2 DESC)vhc_y),
...
SELECT 2021 AS "year", inj.injured, inj.inj_am, kld.killed, kld.kld_am AS killed_am, brg.borough, brg.brg_am AS borough_am, vhc_type.vhc_type, vhc_type.vhc_type_am,  contr_f.contr_f, contr_f.contrf_am AS contr_f_am,
