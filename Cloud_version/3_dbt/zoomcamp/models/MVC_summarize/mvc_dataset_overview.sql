...
T_C AS (
	...
	SELECT collision_id, date_part('year', crash_date) AS "year", borough, vhc_1_code, contr_f_vhc_1 FROM {{ source('MVC_C',src_tbl) }}  
            {%- if not loop.last %}
    UNION ALL
            {% endif -%}
            {% endfor %}
...
T_V AS (    
            {%- for yr in yrs %}
            {%- set src_tbl = 'MVC_V_' ~ yr -%}
	SELECT collision_id, date_part('year', crash_date) AS "year", vhc_type, vhc_year, dr_lic_status FROM {{ source('MVC_V',src_tbl) }}
            {%- if not loop.last %}
    UNION ALL
            {% endif -%}
            {% endfor %}),
T_P AS (   
            {%- for yr in yrs %}
            {%- set src_tbl = 'MVC_P_' ~ yr -%}
	SELECT collision_id, date_part('year', crash_date) AS "year", age, sex FROM {{ source('MVC_P',src_tbl) }}
            {%- if not loop.last %}
    UNION ALL
            {% endif -%}
            {% endfor %}),
